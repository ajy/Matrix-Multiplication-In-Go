...
var Grain int  /* product size below which matmultleaf is used */
var MatSize int
...
func varyGrain(M,N,P int,A,B,C,C4 [][]int){ //Changes the value of Grain and multiplies again and again
	Grain=64*64
	for Grain<(1024*1024*1024){
	before := time.Now()
	matmultS(M, N, P, A, B, C4)
	after := time.Now()
	fmt.Printf("%v,%v\n",Grain,(after.Sub(before)))
	Grain*=2
     }
     	Grain=1024*1024*1024
     	before := time.Now()
	matmultS(M, N, P, A, B, C4)
	after := time.Now()
	fmt.Printf("%v,%v\n",Grain,(after.Sub(before)))
...
func main() {
	...
	randomize(M,P,A)
	identity(P,N,B)
	varyGrain(M,N,P,A,B,C,C4)
	/*fmt.Printf("Execute Standard matmult\n\n")
	before := time.Now()
	seqMatMult(M, N, P, A, B, C)
	after := time.Now()
	fmt.Printf("Standard matrix function done in %v \n\n\n", (after.Sub(before)))*/
